...
from hormuz_throughput.donor_screen import (
    build_screens,
...
def _panel():
    ...
    cols = {}
    cols["strait_of_hormuz"] = np.r_[np.full(500, 50.0), np.full(60, 2.0)]
    cols["riser"] = np.r_[np.full(500, 20.0), np.full(60, 40.0)]
    cols["faller"] = np.r_[np.full(500, 30.0), np.full(60, 15.0)]
    cols["steady"] = np.full(560, 25.0)
...
def test_directional_deviation_signs_match_rise_and_fall():
    ...
    assert "strait_of_hormuz" not in by.index
...
def test_unreliable_donor_is_not_flagged_as_risen():
    ...
    wide.iloc[505, wide.columns.get_loc("riser")] = 99.0
    diag = donor_directional_deviation(
        wide, treated_slug="strait_of_hormuz", history_start=wide.index[0],
        cutoff=wide.index[500], horizon_days=30, min_scored_days=20,
    )
