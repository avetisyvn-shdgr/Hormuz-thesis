...
Both legs train on `panel_aligned.csv`, which starts 2022-01-01:
...
**The −3.7% is conditional on that training window.** The network-adaptation
event panel trains the same two models on the full PortWatch history from
2019-01-01 (Chronos on the trailing 2,048 days) over the identical 130 dates, and
there Chronos gives 7,042 against AR's 6,496 — 8.4% *above* AR rather than below
it. Both specifications are generated into
`experiments/network_adaptation/outputs/hormuz_shortfall_specification_sensitivity.csv`.
Do not quote either percentage without its training start.
...
forecaster?* It does, under both training windows: on the same 130 dates and
observations the shortfall is 92.5-93.0% below counterfactual whichever model and
whichever window is used. What is *not* stable is the two models' agreement with
each other, which is why the magnitude of the model difference cannot be quoted
unconditionally. It is **not** promoted to the primary estimator and is **not**
...
locked-primary transit count on the same 2022-01-01 training window. Directional
agreement may be reported; its precise shortfall magnitude should not carry the
conclusion.
