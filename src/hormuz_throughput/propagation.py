...
class PropagationFit:
    ...
    receiver_loadings: pd.DataFrame
    profile: pd.Series
...
def pair_reallocation(
    ...
    *,
    ...
    """Compatibility shim. The implementation lives in `receiver_equivalence`.

    Moved under the B2 compatibility migration (plan v1.1, section 5). The
    statistic never depended on the ALS fit, so it belongs with the receiver
    work. This path is preserved so existing callers and the legacy import
    keep working; it delegates rather than duplicating, so there is exactly one
    implementation and the two paths cannot drift apart.
    """
    from .receiver_equivalence import pair_reallocation as _implementation
