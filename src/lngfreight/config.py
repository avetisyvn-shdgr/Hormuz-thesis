...
# Repo root = two levels up from this file (src/lngfreight/config.py).
ROOT = Path(__file__).resolve().parents[2]
CONFIG_DIR = ROOT / "config"

load_dotenv(ROOT / ".env")  # populate os.environ from .env if present
...
def path(key: str) -> Path:
    ...
    if not p.suffix:  # it's a directory
