...
def ar_window_statistic(
    ...
    *,
    ...
    # The capacity target carries masked artifact-zeros (NaN). Feed the
    # autoregression a forward-filled copy so the recursive seed does not cascade
    # to an all-NaN path; this uses pre-origin information only. The deviation is
    # still scored against the ORIGINAL observed values (NaN days excluded).
