...
Honesty stance (CLAUDE.md rules 1, 2, 4, 6):
  ...
  * The three real adapters require the heavy PyTorch stack, kept OUT of the
    ...
    runs alone in ``.venv-timesfm`` (``requirements-timesfm.txt``). See
    ``docs/MODERN_TSFM_BENCHMARK.md``. Pin resolved versions before reporting.
...
class TSFMAdapter:
    ...
    name = "base"
    # A fixed quantile grid the model can emit, or None when arbitrary quantiles
    # are supported. Verified at runtime: Chronos-2 supports arbitrary quantiles
    # (native 95%); TimesFM 2.5 and Moirai 2.0 both emit deciles 0.1..0.9, whose
    # widest reliable central interval is 80% (0.1/0.9), NOT 95%.
    ...
    def predict(
        ...
        # All-NaN output signals a broken model/env (e.g. TimesFM 2.5 under the
        # older torch uni2ts pins). Fail loudly rather than write garbage scores.
...
class TimesFM25Adapter(TSFMAdapter):
    ...
    name = "timesfm"
    # The decile quantile grid emitted by the 2.5 continuous quantile head.
    ...
    def __init__(self, max_context: int = 2048, max_horizon: int = 64) -> None:
        ...
        # Verified against the timesfm package shipping the 2.5 API: weights load
        # via the HF-Hub from_pretrained mixin (DEFAULT_REPO_ID =
        # google/timesfm-2.5-200m-pytorch), NOT load_checkpoint().
    ...
    def _predict(
        ...
        # quantile_forecast: [batch, horizon, n]. Verified against TimesFM 2.5:
        # n == 10 == [mean, q0.1, ..., q0.9], i.e. a leading mean column before
        # the 9 deciles. Derive the offset generically rather than assume it.
...
class Moirai2Adapter(TSFMAdapter):
    ...
    def __init__(self, model_name: str = "Salesforce/moirai-2.0-R-small") -> None:
        ...
        # Use the model's own emitted quantile grid (deciles for Moirai 2.0).
...
# Factories so the heavy import only fires when a model is actually instantiated.
...
def admission_test(
    ...
    for _, r in tsfm_agg.iterrows():
        ...
        # Admission requires BOTH legs to pass; an unassessed calibration leg
        # cannot certify admission, but is reported as 'not assessed', not 'failed'.
