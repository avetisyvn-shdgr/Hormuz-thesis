...
# ---------------------------------------------------------------------------
# State loading and separation guards
# ---------------------------------------------------------------------------
...
def assert_not_averaged(frame: pd.DataFrame, july_col: str, august_col: str) -> None:
    ...
    if not np.any(frame[july_col].to_numpy(float) != frame[august_col].to_numpy(float)):
        return  # states agree everywhere here; a "blend" is not distinguishable
    for name in frame.columns:
        if name in (july_col, august_col):
            continue
        column = frame[name]
        if not pd.api.types.is_numeric_dtype(column):
            continue  # labels and dates cannot be a state blend
...
# ---------------------------------------------------------------------------
# Overlap panel and changed-row accounting
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Mapping estimation: proportional, affine, additive
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Residual behaviour and the proportional / non-proportional decomposition
# ---------------------------------------------------------------------------
